 ...
 * Form for creating new inventory items with stock tracking
 */
...
import { useCreateInventoryItem } from '@/lib/api/queries/useInventory'
import { categoryService } from '@/lib/api/services/categoryService'
import { STOCK_UNITS_BY_TYPE, type StockUnit, type ContainerType } from '@/types/inventory'
...
export default function CreateInventoryItemPage() {
  ...
  // Function to load categories (inventory scope)
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      // Set the new category as selected (ensure it's a string for the select)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        // Menu fields with defaults (inventory-only mode)
        price: 0,
        temperature: undefined,
        prep_time: undefined,
        featured: false,
        popular: false,
        available: false,  // Not for sale by default
        notes: formData.notes.trim() || undefined,
        status: 'draft',  // Draft by default since it's inventory-only
        // Inventory fields
      ...
      setError(err.response?.data?.detail || err.message || 'Failed to create inventory item')
    } finally {
  ...
  return (
    <AdminLayout
      ...
      pageSubtitle="Create a new inventory item with stock tracking"
    >
      {/* Top Navigation Bar */}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2">
          <form id="create-inventory-form" onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="sticky top-40 space-y-6">
            ...
            <CardWrapper>
              ...
              <ul className="space-y-3 text-sm text-[#5c5752]">
                ...
                <li className="flex gap-2">
                  <span className="text-[#d4a574]">•</span>
                  <span>Items are created in draft status - publish from menu to sell</span>
                </li>
              </ul>
            </CardWrapper>
